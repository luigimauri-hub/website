(() => {
  ...
  const alts = [
    ...
    'Luigi Mauri durante un incontro in cantiere'
  ];

  if (carousel && slides.length && slides.length === dots.length && toggle) {
    ...
    function start() {
      ...
      if (!paused && !document.hidden) {
        timer = setInterval(() => show(active + 1), 5800);
      }
    }
    ...
    toggle.addEventListener('click', () => {
      ...
      toggle.setAttribute(
        'aria-label',
        paused ? 'Avvia il carosello' : 'Metti in pausa il carosello'
      );
